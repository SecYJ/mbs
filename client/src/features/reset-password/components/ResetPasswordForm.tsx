...
const ValidResetForm = ({ token }: { token: string }) => {
    ...
    return (
        <>
            ...
            <form
                onSubmit={onSubmit}
                className="animate-fade-up animation-duration-800 mt-12 space-y-7 [animation-delay:400ms]"
            >
                <PasswordInput
    ...
    return (
        <section className="animate-fade-up animation-duration-800 mt-14 [animation-delay:320ms]" aria-live="polite">
            ...
            <div className="pt-10">
                <Link
                    to="/forgot-password"
                    className="group relative flex h-12 w-full cursor-pointer items-center justify-center gap-3 border border-(--bone) bg-(--bone) text-[0.72rem] font-semibold tracking-[0.3em] text-black uppercase no-underline transition-all duration-300 hover:border-white hover:bg-white hover:tracking-[0.34em]"
                >
