...
const NotificationMenuTrigger = ({ unreadBadgeLabel, unreadCount }: NotificationMenuTriggerProps) => (
    ...
    >
        ...
        {unreadCount > 0 ? (
            <span className="animate-signal-pulse absolute -top-1 -right-1 min-w-4 rounded-full border border-black bg-(--signal) px-1 text-center text-[0.55rem] leading-4 font-bold text-black">
                {unreadBadgeLabel}
            </span>
        ) : null}
    </PopoverTrigger>
...
const NotificationMenuHeader = ({ children, unreadCount }: NotificationMenuHeaderProps) => (
    <div className="flex items-center justify-between border-b border-(--hairline) px-4 py-3">
        <div>
            <p className="text-[0.74rem] font-semibold tracking-[0.22em] text-(--bone) uppercase">Notifications</p>
