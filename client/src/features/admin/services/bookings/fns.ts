...
export const getAdminBookingsFn = createServerFn({ method: "GET" })
    ...
    .handler(async ({ context, data }) => {
        ...
        return {
            ...
            currentUserRole: context.session.user.role,
...
export const cancelAdminBookingFn = createServerFn({ method: "POST" })
    ...
    .handler(async ({ context, data }) => {
        ...
        const isSuperAdmin = isSuperAdminRole(context.session.user.role);

        if (booking.userId !== context.session.user.id && !isSuperAdmin) {
                ...
                .where(
                    isSuperAdmin
                        ...
                        : and(
                              ...
                              eq(bookings.userId, context.session.user.id),
