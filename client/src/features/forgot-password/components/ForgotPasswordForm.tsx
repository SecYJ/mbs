...
export const ForgotPasswordForm = () => {
    const { form, onSubmit, isPending, isSuccess, sentToEmail, reopen } = useForgotPassword();

    if (isSuccess && sentToEmail) {
        return (
            <section
                aria-live="polite"
                className="animate-fade-up animation-duration-800 mt-12 space-y-8 [animation-delay:0ms]"
            >
                <div className="flex items-start gap-4 border-l border-(--gold) pl-5">
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-[0.86rem] leading-relaxed text-(--bone-muted)">
                            ...
                            recovery link is on its way. The link expires in minutes.
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center justify-between gap-4">
                    ...
                    <button
                        type="button"
                        onClick={reopen}
                        className="shrink-0 cursor-pointer text-[0.7rem] font-semibold tracking-[0.24em] text-(--bone-muted) uppercase transition-colors hover:text-(--gold)"
                    >
                        Send another
                    </button>
                </div>
    ...
    return (
        <form
            onSubmit={onSubmit}
            className="animate-fade-up animation-duration-800 mt-12 space-y-8 [animation-delay:400ms]"
        >
            <Controller
            ...
            <div className="pt-4">
                <FormStateSubscribe
                    ...
                    render={({ isSubmitting }) => (
                        ...
                        >
                            <span>{isPending ? "Dispatching" : "Dispatch link"}</span>
                            <ArrowRight
