...
import { Ban, CalendarDays, ChevronLeft, ChevronRight, History, Plus, X } from "lucide-react";
import { z } from "zod";
...
function BookingsPage() {
    ...
    return (
        <div className="space-y-6">
            ...
            <BookingHistoryList bookings={data.history} selectedBookingId={bookingId} />
...
const BookingHistoryList = ({
    ...
    const selectedHistoryBooking = selectedBookingId
        ? bookings.find((booking) => booking.id === selectedBookingId)
        : undefined;
    const previewBookings =
        selectedHistoryBooking && !bookings.slice(0, 8).some((booking) => booking.id === selectedHistoryBooking.id)
            ? [
                  selectedHistoryBooking,
                  ...bookings.filter((booking) => booking.id !== selectedHistoryBooking.id).slice(0, 7),
              ]
            : bookings.slice(0, 8);
