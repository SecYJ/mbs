...
const CalendarControls = () => {
    ...
    const { calendar, viewContainsToday, todayButtonLabel } = useBookingCalendarStore(
        useShallow(({ calendar: calendarInstance, visibleRange }) => {
            ...
            return { calendar: calendarInstance, viewContainsToday: isTodayVisible, todayButtonLabel: buttonLabel };
        }),
    );
    ...
    return (
        <div className="gap-y-5 lg:flex lg:items-center lg:justify-between lg:space-y-0 animate-fade-up animation-duration-700 [animation-delay:200ms]">
                ...
                <div className="flex items-stretch divide-x divide-(--hairline)">
                    {bookingCalendarViews.map((viewKey) => (
                        <Link
                            ...
                            className="relative cursor-pointer px-4 py-1 text-[0.66rem] font-semibold tracking-[0.28em] uppercase transition-colors disabled:cursor-default text-(--bone-dim) hover:text-(--bone-muted)"
                            activeProps={{
                                className: "text-(--bone)",
                            }}
                        >
