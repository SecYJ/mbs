...
export const ReservationDetails = ({ dialogState }: { dialogState: ReservationViewState }) => {
    ...
    } = useReservationView({ dialogState, closeReservation });
    ...
    return (
        <>
            <div className="space-y-2">
                ...
                <h2 className="display-italic text-[1.75rem] leading-[1.05] font-normal text-(--bone)">{eventTitle}</h2>
            ...
            {(canManage || cancelError) && (
                <div className="mt-6 border-t border-(--hairline) pt-5">
                    ...
                    {canManage && !cancel.visible && (
                        <ReservationActionButtons
                            isCancelling={isCancelling}
                            onCancel={requestCancelBooking}
                            onEdit={() => onReservationEditing(true)}
                        />
                    )}
                    {canManage && cancel.visible && (
                        <div className="space-y-3">
                            ...
                            <div className="flex items-center justify-end gap-3">
                                ...
                                <button
                                    type="button"
                                    onClick={() => confirmCancelBooking(cancel.reason)}
                                    disabled={isCancelling}
